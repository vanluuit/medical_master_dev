...
<div class="container-fluid">
        <!-- Column -->
            ...
            <div class="card">
              <?php echo Form::open(array('route' => 'notification.store', 'id'=>'form_ajax', 'enctype'=>'multipart/form-data')); ?>
              ...
              <div class="card-body">
                <div class="row">
                  <div class="col-md-6">
                    ...
                    <div class="form-group row">
                      <div class="col-md-12">
                         <?php echo Form::select('category_id', $cate, '', ['placeholder' => '学会選択', 'class'=>'form-control select2 change_new_sl filter_in', 'id'=>'category_id']); ?>

                      </div>
                    </div>
                    <div class="form-group row">
                      <div class="col-md-12">
                         <?php echo Form::select('category_new_id', $cate_new, '', ['placeholder' => 'category new', 'class'=>'form-control select2 change_new_sl', 'id'=>'category_new_id']); ?>

                      </div>
                    </div>
                    <div class="form-group row">
                      <div class="col-md-12">
                         <?php echo Form::select('new_id', $news, '', ['placeholder' => ' new', 'class'=>'form-control select2', 'id'=>'new_id']); ?>
                  ...
                  <div class="col-md-6">
                    <p>user push list <span>total ( <span id="total_ajax"><?php echo e(count($users)); ?></span> )</span></p>
                    <div id="result_user">
...
<?php $__env->startSection('script'); ?> 
    <script>
        ...
        $(document).ready(function(){
            var validateForm = $('#validate').validate({
                ...
                messages: {
                    ...
                    // 'category_new_id': {
                    //     required:  "categoryが間違っています。",
                    // },
                    // 'category_id': {
                    //     required:  "学会選択が間違っています。",
                    // },
                    // 'new_id': {
                    //     required:  "newが間違っています。",
                    // },
                },
        ...
        $(document).on('change','.change_new_sl', function(){
          var category_id = $('#category_id').val();
          var category_new_id = $('#category_new_id').val();
          $.ajax({url: "<?php echo e(route('news.by.category')); ?>?category_id="+category_id+'&category_new_id='+category_new_id, success: function(result){
            var ar = JSON.parse(result);
            // if(ar.length > 0){
              var str ='<option selected=\"selected\" value=\"\">New</option>\"';
        ...
        $(document).on('change','.filter_in', function(){
          ...
          $.ajax({url: "<?php echo e(route('user.ajax.get')); ?>?"+$('#form_ajax').serialize(), 
      ...
      $(document).on('change','#category_id', function(){
          $.ajax({url: "<?php echo e(route('member.ajax.getlist')); ?>?"+$('#form_ajax').serialize(), 
            success: function(result){
              var str = '';
              var dl = JSON.parse(result);
              for(let i = 0; i < dl.length; i++){
          ...
          $.ajax({url: "<?php echo e(route('careers.ajax.getlist')); ?>?"+$('#form_ajax').serialize(), 
            success: function(result){
              var str = '';
              var dl = JSON.parse(result);
              for(let i = 0; i < dl.length; i++){
        ...
        $.ajax({url: "<?php echo e(route('provinces.ajax.getlist')); ?>?"+$('#form_ajax').serialize(), 
            success: function(result){
              var str = '';
              var dl = JSON.parse(result);
              for(let i = 0; i < dl.length; i++){
    ...
    $(document).on('click','.pushPreview', function(){
      $('#loading').html('<img src="/loading.gif" />');
          $.ajax({url: "<?php echo e(route('notification.push.preview')); ?>?"+$('#form_ajax').serialize(), 
            success: function(result){
